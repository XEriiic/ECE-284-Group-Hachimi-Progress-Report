...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, ss_bool=0, dilation=1, groups=1, bias=False):
    ...
    def forward(self, x):
        ...
        return F.conv2d(x, weight_q, self.bias, self.stride, self.padding, self.dilation, self.groups) if self.ss_bool == 0 else self.ss(F.conv2d(x, weight_q, self.bias, self.stride, self.padding, self.dilation, self.groups))
...
class TrainableSoftshrink(nn.Module):
    def __init__(self, initial_lambda=0.5):
        super().__init__()

        # trainable underlying parameter (float)
        self.raw_lambda = nn.Parameter(torch.tensor(initial_lambda, dtype=torch.float32))

        # integer bounds
        self.L_MIN = -(2**15)      # -32768
        self.L_MAX = (2**15) - 1   # 32767

    def forward(self, x):
        # Convert float parameter to integer during forward pass
        lambd_int = torch.round(self.raw_lambda)

        # enforce |lambda| >= 1
        # if rounded value is 0, snap to +/-1 based on raw_lambda's sign
        lambd_int = torch.where(
            torch.abs(lambd_int) < 1,
            torch.sign(lambd_int + 1e-6),   # avoids sign(0) = 0
            lambd_int
        )

        # final clamp within allowed range
        lambd_int = torch.clamp(lambd_int, self.L_MIN, self.L_MAX)

        # Softshrink with integer lambda
        mask1 = x > lambd_int
        mask2 = x < -lambd_int

        out = torch.zeros_like(x)
        out += mask1.float() * (x - lambd_int)
        out += mask2.float() * (x + lambd_int)
