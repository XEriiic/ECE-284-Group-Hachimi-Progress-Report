...
cfg = {
    ...
    'VGG16_quant_16x16': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 'A_16', 'B_16', 'C_16', 'M', 512, 512, 512, 'M'],
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                conv_layer = QuantConv2d(256, 16, kernel_size=3, padding=1)
                conv_layer.act_alq = act_quantization(2)
                conv_layer.bit = 4
                layers += [conv_layer,
                           nn.BatchNorm2d(16),
                           nn.ReLU(inplace=True)]
                ...
                conv_layer = QuantConv2d(16, 16, kernel_size=3, padding=1)
                conv_layer.act_alq = act_quantization(2)
                conv_layer.bit = 4
                layers += [conv_layer,
                           nn.ReLU(inplace=True)]
                ...
                conv_layer = QuantConv2d(16, 256, kernel_size=3, padding=1)
                conv_layer.act_alq = act_quantization(2)
                conv_layer.bit = 4
                layers += [conv_layer,
                           nn.BatchNorm2d(256),
...
def VGG16_quant_16x16(**kwargs):
    model = VGG_quant(vgg_name = 'VGG16_quant_16x16', **kwargs)
    return model
